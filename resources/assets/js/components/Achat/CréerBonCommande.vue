...
<div>
    ...
    <div class="container h-100">
        ...
        <h1 class="text-center my-5">Créer Bon Commande</h1>
        <div class="col-md-4 offset-md-4">
            <div class="form-group">
                <label for="">Sélection la D.A Pour Laquelle Vous Souhaitez créer un Bon de Commande</label>
                <select name="type" id="" class="form-control" v-model="demandeSelectionnée">
                    <option :value="demande" v-for="demande in demande_achats">{{ demande.numero }}</option>
                </select>
            </div>
        </div>
...
export default {
    ...
    methods: {
        recupereLesDemandes(){
            axios.get('/api/demande-achat/en-cours').then(response => {
                this.demande_achats = response.data
            });
            
        },
        créeLeBon(){
            axios.post('/api/bon-commande/créer', this.demandeSelectionnée ).then(response => {
                this.isLoading = true;
                this.$notify({
                  group: 'foo',
                  title: 'Succès',
                ...
                setTimeout(() => {
                    window.location.replace("/module-achat/dossier-bon-commande/" + response.data.id); 
                }, 4500);
            }).catch(error => {
