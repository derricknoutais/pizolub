...
export default {
    ...
    methods: {
        recupereLesDemandes(){
            axios.get('/api/demande-fabrication/en-cours').then(response => {
                this.demande_achats = response.data
            });
            
        },
        créeLeBon(){
            axios.post('/api/bon-fabrication/créer', this.demandeSelectionnée ).then(response => {
                this.isLoading = true;
                this.$notify({
                  group: 'foo',
                  title: 'Succès',
                ...
                setTimeout(() => {
                    window.location.replace("/module-fabrication/dossier-bon-fabrication/" + response.data.id); 
                }, 4500);
            }).catch(error => {
